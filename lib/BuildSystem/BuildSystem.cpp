  ...
  void constructCommandLineArgs(StringRef outputFileMapPath,
    ...
    for (const auto& nameAndAlias: moduleAliases) {
      // E.g. `-module-alias Foo=Bar`
        shellEscaped("ESQQ llbuild::BuildSystem::constructCommandLineArgs - MODULE ALIAS ARG: %s\n" + nameAndAlias);
      ...
      ArrayRef<std::pair<StringRef, StringRef>> values) override {
        if (name == "module-alias") {
          ...
          for (auto pair: values) {
            ...
            shellEscaped("ESQQ llbuild::BuildSystem::configureAttribute - MODULE ALIAS ARG FORMATTED: %s\n" + formatted.str());
          }
