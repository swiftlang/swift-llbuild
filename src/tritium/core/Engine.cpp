...
#include "tritium/support/Defer.h"
...
using namespace tritium::core::internal;
...
class EngineImpl {
  ...
  /// The action cache.
  std::unique_ptr<ActionCache> actionCache;
  ...
  LabelTrie<ProviderID> ruleProviderMap;
  LabelTrie<ProviderID> artifactProviderMap;
...
public:
  EngineImpl(EngineConfig config, std::unique_ptr<ActionCache>&& cache)
      : cfg(config), actionCache(std::move(cache)) {
...
private:
  ...
  void processTaskNextState(TaskInfo& taskInfo, const TaskNextState& next) {
    ...
    case TaskNextState::kWait: {
      ...
      if (unresolvedInputs) {
        ...
        enqueueReadyTask(taskInfo, batchState.context(), inputs);
      }
      break;
    }
      ...
      for (auto id : taskInfo.requestedBy) {
        ...
        if (--rtask.waitCount == 0) {
          ...
          if (rtask.nextState.has_wait()) {
            ...
            enqueueReadyTask(rtask, batchState.context(), inputs);
          } else {
  ...
  public:
    RootTask(EngineImpl& engine, uint64_t requestID, const Label& artifact,
             std::shared_ptr<BuildContext> buildContext)
        : Task(Task::Properties()), engine(engine), artifact(artifact), buildContext(buildContext) {
      requestName.add_components("__build__");
      requestName.set_name(std::to_string(requestID));
    }
      ...
      if (!engine.initialized) {
        ...
        if (engine.initTask.has_value()) {
          ...
          engine.taskInfos.at(taskID).requestedTaskInputs.insert(
              {initTaskID, TaskInputRequest{false, name()}});
        } else {
          ...
          result<uint64_t, Error> res;
          {
            ...
            if (res) {
              engine.taskInfos.at(taskID).requestedTaskInputs.insert(
                  {*res, TaskInputRequest{false, name()}});
            }
          }
  ...
  void executeLane(uint32_t laneNumber) {
    ...
    std::string threadName = "tritium-engine-" + std::to_string(laneNumber);
...
Engine::Engine(EngineConfig config, std::unique_ptr<ActionCache>&& cache, std::unique_ptr<RuleProvider>&& init)
    : impl(new EngineImpl(config, std::move(cache))) {
