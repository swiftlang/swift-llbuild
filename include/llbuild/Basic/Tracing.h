...
if (__builtin_available(macOS 10.14, iOS 12.0, *)) os_signpost_interval_begin(getLog(), signpost_id(), name, ##__VA_ARGS__); \
}

/// End an interval if tracing is enabled.
#define LLBUILD_TRACE_INTERVAL_END(name, ...) { \
if (__builtin_available(macOS 10.14, iOS 12.0, *)) os_signpost_interval_end(getLog(), signpost_id(), name, ##__VA_ARGS__); \
}

/// Trace an event without duration at a point in time.
#define LLBUILD_TRACE_POINT(name, ...) { \
if (__builtin_available(macOS 10.12, iOS 10.0, *)) os_log(getLog(), ##__VA_ARGS__); \
}
