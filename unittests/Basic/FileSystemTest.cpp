...
TEST(FileSystemTest, basic) {
  ...
  {
    std::error_code ec;
    llvm::raw_fd_ostream os(tempPath.str(), ec, llvm::sys::fs::OF_Text);
    EXPECT_FALSE(ec);
    os << "Hello, world!";
    os.close();
  }
  ...
  auto ourFileInfo = fs->getFileInfo(tempPath.str().str());
  ...
  auto ourFileContents = fs->getFileContents(tempPath.str().str());
...
TEST(FileSystemTest, testRecursiveRemoval) {
  ...
  {
    std::error_code ec;
    llvm::raw_fd_ostream os(file.str(), ec, llvm::sys::fs::OF_Text);
    EXPECT_FALSE(ec);
    os << "Hello, world!";
    os.close();
  }
  ...
  {
    std::error_code ec;
    llvm::raw_fd_ostream os(dir.str(), ec, llvm::sys::fs::OF_Text);
    EXPECT_FALSE(ec);
    os << "Hello, world!";
    os.close();
  }
...
TEST(FileSystemTest, testRecursiveRemovalDoesNotFollowSymlinks) {
  ...
  {
    std::error_code ec;
    llvm::raw_fd_ostream os(otherFile.str(), ec, llvm::sys::fs::OF_Text);
    EXPECT_FALSE(ec);
    os << "Hello, world!";
    os.close();
  }
